...
HOXB6_1 possesses a highly variable methylation profile (see section 5), which may increase discriminatory power.
```{r}
...
As discussed in the report, we reasoned that genes displaying high methylation variability may posses increased discriminatory power relative to genes with lower variability.
...
We assessed linearity in the logit for continuous predictors using a Box-Tidwell-type approach. For binary predictors such as label_gender, the linearity assumption is inherently valid and therefore does not require testing.
...
df_tested$label_age <- df_tested$label_age + abs(min(df_tested$label_age)) + 0.01  #Box-Tidwell test requires positive values
df_tested$HOXB6_1 <- df_tested$HOXB6_1 + abs(min(df_tested$HOXB6_1)) + 0.01  #Box-Tidwell test requires positive values
...
```
Although high VIF values are expected in models with interaction terms, interpretation should be context-dependent. As no predictors were statistically significant in the exploratory interaction model, it was discarded and model_2 was selected as the best-fitting model.
