...
### Subtask:
Voer Principal Component Analysis (PCA) uit op de `X_train` dataset om de dimensionaliteit te verminderen. We zullen beginnen met het behouden van 2 of 3 hoofdcomponenten voor visualisatie, en later kunnen we dit aanpassen op basis van de verklaarde variantie.
...
"""**Reasoning**:
Now that the variances for each gene in `X_train` have been calculated, I need to define a variance threshold to identify and subsequently remove the genes with low variance from both `X_train` and `X_test`. I will use a threshold of 0.0001 to filter out genes that contribute very little to the overall data variability.
...
via trial and error: 2000 genes = C of
"""
...
| :------------------------------ | :-------------------- | :--------------------- |
| **Trainingsnauwkeurigheid** | 1.00 | 1.00 |
| **Testnauwkeurigheid** | **0.93** | **0.88** |
| Beste Hyperparameters | N.v.t. | `{'n_estimators': 200, 'min_samples_split': 5, 'min_samples_leaf': 1, 'max_features': 0.8, 'max_depth': 30}` |
...
| Precisie | 1.00 | 0.89 | 0.92 |
| Recall | 0.90 | 1.00 | 0.88 |
| F1-score | 0.95 | 0.94 | 0.90 |
| **Getunede Random Forest** |
| Precisie | 0.90 | 0.88 | 0.88 |
...
# We zullen de getunede Random Forest's X_train_filtered en y_train gebruiken
# Deze bevat de top 2000 meest variabele genen.

# 1. Definieer het hyperparameter grid
param_grid_lr = {
    'C': [0.001, 0.01, 0.1, 1, 10, 100],  # Regularisatiesterkte
    'penalty': ['l1', 'l2'],           # Type regularisatie
    'solver': ['liblinear']            # Solver, kies compatibel met penalty
}
...
grid_search_lr = GridSearchCV(
    ...
    cv=5,                 # 5-fold cross-validatie
    scoring='accuracy',   # Optimaliseer voor nauwkeurigheid
    n_jobs=-1,            # Gebruik alle beschikbare CPU-kernen
    verbose=1             # Toon voortgang
)

print("GridSearchCV voor Logistische Regressie geïnitialiseerd.")

print("Starten van GridSearchCV voor Logistische Regressie...")
grid_search_lr.fit(X_train_filtered, y_train)

print("GridSearchCV voltooid.")

# Haal de beste parameters en beste score op
best_params_lr = grid_search_lr.best_params_
best_score_lr = grid_search_lr.best_score_

print(f"\nBeste hyperparameters gevonden voor Logistische Regressie: {best_params_lr}")
print(f"Beste cross-validatie nauwkeurigheid voor Logistische Regressie: {best_score_lr:.4f}")
...
print(f"Trainen van Logistische Regressie met beste hyperparameters: {best_params_lr}")
...
print("Getuned Logistisch Regressie model succesvol getraind.")
...
print(f"\nTrainingsnauwkeurigheid (Getunede Logistische Regressie): {train_accuracy_lr_tuned:.2f}")
...
print(f"Testnauwkeurigheid (Getunede Logistische Regressie): {test_accuracy_lr_tuned:.2f}")


print("\nClassificatie Rapport voor Test Data (Getunede Logistische Regressie):")
...
plt.title('Confusion Matrix voor Getuned Logistisch Regressie Model')
...
| Metriek                       | Initieel Logistisch Regressie Model | Getuned Logistisch Regressie Model |
| :---------------------------- | :--------------------------------- | :-------------------------------- |
| **Beste Hyperparameters**     | N.v.t.                             | `{'C': 100, 'penalty': 'l1', 'solver': 'liblinear'}` |
| **Trainingsnauwkeurigheid**   | 1.00                               | 1.00                              |
| **Testnauwkeurigheid**        | 0.93                               | 0.93                              |
| **Colon_and_rectum_cancer Precisie** | 1.00                               | 1.00                              |
| **Colon_and_rectum_cancer Recall**   | 0.90                               | 0.93                              |
| **Colon_and_rectum_cancer F1-score** | 0.95                               | 0.96                              |
| **Lung_cancer Precisie**      | 0.89                               | 0.86                              |
| **Lung_cancer Recall**        | 1.00                               | 1.00                              |
| **Lung_cancer F1-score**      | 0.94                               | 0.93                              |
| **Stomach_cancer Precisie**   | 0.92                               | 0.96                              |
